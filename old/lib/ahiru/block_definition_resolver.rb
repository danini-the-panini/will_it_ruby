module Ahiru
  class BlockDefinitionResolver
    attr_reader :malleable_ducks

    def initialize(args_exp, expressions, parent_scope)
      @args_exp = args_exp
    ...
    def create_scope
      BlockScope.new(@parent_scope.world, @parent_scope.t_self.instance_type, @parent_scope, @parent_scope.ceiling_scope, @parent_scope.method_scope)
    end

    def resolve
      pargs, kwargs, block = process_definition_args(@args_exp == 0 ? [] : @args_exp[1..-1])
      ...
      Block.new(final_return_type, final_pargs, final_kwargs, final_block, free_types)
    end
